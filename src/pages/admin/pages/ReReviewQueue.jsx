...
import { handleItemApproved, handleItemRejected } from "@/lib/notificationEvents"
import { CalendarIcon, CheckCircle2, Loader2, XCircle, ClipboardList } from "lucide-react"
      ...
      if (itemIds.length > 0) {
        try {
          const { data: itemsData, error: itemsErr } = await supabase
            .from("items")
            .select("item_id, title, main_image_url, user_id, item_status, created_at, review_update_note")
            .in("item_id", itemIds)
            ...
            const merged = (data || []).map((row) => {
              ...
              return {
                ...
                items: {
                  ...
                  review_update_note: base.review_update_note ?? (found && found.review_update_note) ?? null,
                },
              }
            })
            setRows(merged)
  ...
  const takeAction = async ({ row, approve, notes }) => {
    ...
    try {
      ...
      if (approve) {
        ...
        if (itemErr) {
          const isEnumError = /invalid input value for enum|invalid input value/i.test(itemErr.message || "")
          if (isEnumError) {
      ...
      if (!approve) {
        try {
          ...
          console.warn("Failed to insert rejection violation log", vioErr)
        }
        try {
      ...
      toast.success(approve ? "Re-review approved" : "Re-review rejected")
      fetchQueue()
    } catch (e) {
    ...
    <AdminLayout>
      <div className="p-6 max-w-[1600px] mx-auto">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Re-review Requests</h1>
            <p className="text-sm text-gray-600">Review owner requests to restore banned items</p>
          </div>
          <div className="ml-auto flex items-center gap-2">
            <div className="text-sm text-gray-700">
              Pending: <span className="font-semibold">{pendingCount}</span>
            </div>
            <Button onClick={fetchQueue} variant="outline" className="border-gray-300 bg-transparent">
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b bg-gray-50">
                  ...
                  <th className="p-4 text-xs font-semibold text-gray-600 uppercase tracking-wider">Item</th>
                  <th className="p-4 text-xs font-semibold text-gray-600 uppercase tracking-wider">Requested By</th>
                  <th className="p-4 text-xs font-semibold text-gray-600 uppercase tracking-wider">Requested At</th>
              ...
              <tbody>
                  ...
                  <tr>
                    <td colSpan={6} className="p-8 text-center text-gray-500">
                      <div className="flex items-center justify-center gap-2">
                        <Loader2 className="h-4 w-4 animate-spin text-[#FFAB00]" /> Loading
                      </div>
                    </td>
                  </tr>
                )}
                ...
                {!loading &&
                  filteredRows.map((r) => (
                    <tr key={r.request_id} className="border-b hover:bg-orange-50/30">
                      <td className="p-4">
                        <button
                          type="button"
                          onClick={() => {
                          ...
                          className="w-12 h-12 rounded-md overflow-hidden bg-gray-100 flex-shrink-0 border border-gray-200 p-0 hover:opacity-80 transition-opacity"
                          title="View details"
                          aria-label={`View details for ${r.items?.title || r.item_id}`}
                        >
                      ...
                      <td className="p-4">
                        <div className="flex flex-col">
                          <span className="font-medium text-gray-900 line-clamp-1">{r.items?.title || "Item"}</span>
                          <span className="text-xs text-amber-700 font-mono">{r.item_id}</span>
                          {r.items?.review_update_note && (
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          <CalendarIcon className="w-4 h-4 text-gray-400" /> {new Date(r.requested_at).toLocaleString()}
                        </div>
                      </td>
                      <td className="p-4">
                        {r.status === "pending" && (
        ...
        {viewItem && <RequestDetailsDialog row={viewItem} onClose={() => setViewItem(null)} />}
        {actionRow && (
          <ActionNotesDialog
            mode={actionType}
            notes={adminNotes}
            ...
            onConfirm={() => {
              takeAction({ row: actionRow, approve: actionType === "approve", notes: adminNotes })
              setActionRow(null)
              setActionType(null)
            }}
          />
    ...
    const load = async () => {
      ...
      try {
        const { data, error } = await supabase
          ...
          .select("violation_id, reason, details, created_at, reported_by")
          .eq("item_id", row.item_id)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-3xl max-h-[90vh] bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col overflow-hidden">
        <div className="px-6 py-4 border-b bg-gradient-to-r from-orange-50 to-amber-50 flex items-center justify-between">
          <div>
            <div className="text-lg font-semibold text-gray-900">Request Details</div>
            <div className="text-sm text-gray-600">{row.items?.title}</div>
          </div>
        ...
        <div className="p-6 overflow-y-auto">
          {row.items?.main_image_url && (
            <div className="mb-6 bg-white rounded-xl p-4 shadow-sm border border-gray-200">
              <img
                src={row.items.main_image_url || "/placeholder.svg"}
                alt={row.items?.title || "Item image"}
          ...
          <div className="mb-4">
            <div className="text-sm text-gray-700">
              ...
              <span className="font-mono text-amber-700">{row.item_id}</span>
            </div>
            ...
            <div className="text-sm text-gray-700">
              <span className="font-medium">Requested at:</span> {new Date(row.requested_at).toLocaleString()}
            </div>
            <div className="text-sm text-gray-700">
              <span className="font-medium">Current item status:</span> {row.items?.item_status}
            </div>
            {row.items?.review_update_note ? (
              <div className="mt-2 text-sm text-gray-700">
                <span className="font-medium">Owner change summary:</span>{" "}
                <span className="whitespace-pre-wrap">{row.items.review_update_note}</span>
              </div>
            ) : (
              <div className="mt-2 text-sm text-gray-500">No change summary provided for this request.</div>
            )}
            {row.admin_notes && (
              <div className="mt-2 text-sm text-gray-700">
                <span className="font-medium">Admin notes:</span>{" "}
                <span className="whitespace-pre-wrap">{row.admin_notes}</span>
              </div>
            )}
          </div>
          <div className="mt-4">
            <h3 className="text-sm font-semibold text-gray-800 mb-2">Violation History</h3>
            <div className="bg-white border rounded-xl">
              {loading && <div className="p-4 text-sm text-gray-500">Loading…</div>}
              {!loading && violations.length === 0 && (
                <div className="p-4 text-sm text-gray-500">No violations recorded</div>
              )}
              {!loading && violations.length > 0 && (
                <ul className="divide-y">
                  {violations.map((v) => (
                    <li key={v.violation_id} className="p-4">
                      <div className="text-sm font-medium text-gray-900">{v.reason}</div>
                      {v.details && <div className="text-sm text-gray-700 mt-1">{v.details}</div>}
...
function ActionNotesDialog({ mode, notes, setNotes, onCancel, onConfirm }) {
  ...
  const ctaClass =
    mode === "approve" ? "bg-green-600 hover:bg-green-700 text-white" : "bg-red-600 hover:bg-red-700 text-white"
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-xl bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b bg-gradient-to-r from-orange-50 to-amber-50">
          ...
          <div className="text-sm text-gray-600">Add optional admin notes for audit trail and owner visibility.</div>
        </div>
        <div className="p-6">
